...
class Manufacturing_Simulator:
    ...
    def get_seller_params(self):
        params = [{} for _ in range(self.num_agents)]

        start_time = self.t - self.history_length
        for i in range(self.num_agents):
            params[i]['avg_spot_price'] = np.mean(self.spot_price[..., start_time:self.t], axis=1)
            params[i]['inv'] = self.inv[i, :, self.t]
            params[i]['waste_inv'] = self.waste_inv[i, :, self.t]
            params[i]['alpha'] = self.alpha
            params[i]['phi'] = self.phi[i]
    ...
    def get_buyer_param(self):
        params = [{} for _ in range(self.num_agents)]
        
        for i in range(self.num_agents):
            params[i]['inv'] = self.inv[i, :, self.t]
            params[i]['waste_inv'] = self.waste_inv[i, :, self.t]
            params[i]['max_inv'] = self.init_inv[i, :] * 15. # TODO
            params[i]['max_waste_inv'] = self.init_inv[i, :] * 15. # TODO
            params[i]['beta'] = self.beta
            params[i]['prices'] = self.price[:, :, self.t]
            params[i]['waste_prices'] = self.waste_price[:, :, self.t]
            params[i]['spot_price'] = self.spot_price[:, self.t]
    ...
    def get_trans_param(self):
        ...
        params = [{} for _ in range(self.num_agents)]
        for i in range(self.num_agents):
            params[i]['delta'] = self.delta
            params[i]['prices'] = self.price[i, :, self.t]
            params[i]['waste_prices'] = self.waste_price[i, :, self.t]
            params[i]['uc'] = self.uc_p[:, self.t]
            params[i]['tx'] = self.tx_p[:, self.t]
            params[i]['balanced_inv'] = self.inv_buy[i,:,self.t]
            params[i]['balanced_winv'] = self.waste_inv_buy[i,:,self.t]
